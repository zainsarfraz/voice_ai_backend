...
async def create_assistant(assistant: AssistantCreate, session: SessionDep, current_user: User = Depends(get_current_user)):
    """
    Create new user assistant based on the settings
    """
    assistant = create_assistant_service(assistant_create=assistant, session=session, current_user=current_user)
    return assistant
